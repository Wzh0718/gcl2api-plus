    ...
    async def process_stream(self) -> AsyncGenerator[bytes, None]:
        ...
        while self.current_attempt < self.max_attempts:
            ...
            try:
                ...
                # 无论本轮流式响应被完整消费还是中途放弃（break/return），
                # 都显式关闭底层 body_iterator，释放上游连接
                if isinstance(response, StreamingResponse):
                    await aclose_quietly(response.body_iterator, "anti-truncation")
...
async def apply_anti_truncation_to_stream(
    ...
    # 返回包装后的流式响应
    return ClosingStreamingResponse(processor.process_stream(), media_type="text/event-stream")
