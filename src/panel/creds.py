...
async def _recover_credential_after_success(
    filename: str,
    *,
    mode: str,
    model_name: str,
) -> None:
    """Reconcile persisted error/health state after a real successful probe."""
    ...
    if mode == "antigravity":
        # 面板测试会绕过轮换选择，因此它可以证明一个被误禁用账号已恢复。
...
async def get_creds_status_common(
    ...
    for summary in result["items"]:
        ...
        if mode == "geminicli":
            ...
            cred_info["proxy_mode"] = summary.get("proxy_mode", "inherit")
            cred_info["proxy_group_id"] = summary.get("proxy_group_id")
            cred_info["bound_proxy_node_id"] = summary.get("bound_proxy_node_id")
            cred_info["bound_proxy_node_name"] = summary.get("bound_proxy_node_name")
            cred_info["egress_ip"] = summary.get("egress_ip")
            cred_info["egress_country"] = summary.get("egress_country")
            cred_info["binding_status"] = summary.get("binding_status", "unchecked")
            cred_info["eligibility_status"] = summary.get(
                "eligibility_status", "unchecked"
            )
            cred_info["eligibility_checked_at"] = summary.get(
                "eligibility_checked_at"
            )
            cred_info["last_403_category"] = summary.get("last_403_category")
...
async def verify_credential_project_common(filename: str, mode: str = "geminicli") -> JSONResponse:
    ...
    if mode == "antigravity":
        network = await storage_adapter.resolve_credential_network(filename, mode=mode)
    ...
    if project_id or subscription_tier:
        ...
        if mode == "antigravity":
            await _recover_credential_after_success(
                filename,
                mode=mode,
...
async def get_model_call_stats(
    ...
    """
    获取「凭证 × 模型」调用统计，并聚合出按账号和按模型的成功率。

    统计来自 record_api_call_result 的累计计数（credential_model_stats 表），
    口径为每次上游调用尝试（含重试中的单账号失败）。
    date 传入 YYYY-MM-DD 时返回该日（北京时间）的按日统计，缺省返回全量累计。

    Returns:
        stats: 原始「凭证 × 模型」行（含上游/网关耗时累计）
        by_credential: 按账号聚合（含 success_rate 百分比）
        by_model: 按模型聚合（含 success_rate 百分比）
        by_task_type: 按任务类型聚合（image=生图 / chat=普通，按模型名分类）
        date: 实际生效的日期（全量时为 None）
    """
    try:
        mode = validate_mode(mode)
        if date is not None:
        ...
        storage_adapter = await get_storage_adapter()
        get_stats = getattr(storage_adapter._backend, "get_model_stats", None)
        rows = await get_stats(mode=mode, date=date) if get_stats else []

        # 耗时字段：老后端/老数据可能没有，缺省按 0 处理
        def _latency_fields(row: dict) -> dict:
            return {
                "upstream_total_seconds": row.get("upstream_total_seconds") or 0.0,
                "upstream_timed_count": row.get("upstream_timed_count") or 0,
                "gateway_total_seconds": row.get("gateway_total_seconds") or 0.0,
                "gateway_timed_count": row.get("gateway_timed_count") or 0,
            }

        def aggregate(source_rows: List[dict], key: str) -> List[dict]:
            grouped: dict = {}
            for row in source_rows:
                entry = grouped.setdefault(
                    row[key],
                    {
                        ...
                        "user_email": None, "tier": None,
                        "upstream_total_seconds": 0.0, "upstream_timed_count": 0,
                        "gateway_total_seconds": 0.0, "gateway_timed_count": 0,
                    },
                )
                entry["success_count"] += row["success_count"]
                entry["failed_count"] += row["failed_count"]
                latency = _latency_fields(row)
                for field, value in latency.items():
                    entry[field] += value
                if not entry["user_email"] and row.get("user_email"):
                    entry["user_email"] = row["user_email"]
                if not entry["tier"] and row.get("tier"):
                    entry["tier"] = row["tier"]
            result = []
            for name, counts in grouped.items():
                total = counts["success_count"] + counts["failed_count"]
                item = {
                    key: name,
                    "success_count": counts["success_count"],
                    "failed_count": counts["failed_count"],
                    "total": total,
                    "success_rate": round(100.0 * counts["success_count"] / total, 1) if total else None,
                    "upstream_total_seconds": counts["upstream_total_seconds"],
                    "upstream_timed_count": counts["upstream_timed_count"],
                    "gateway_total_seconds": counts["gateway_total_seconds"],
                    "gateway_timed_count": counts["gateway_timed_count"],
                }
                # 邮箱和套餐级别只对账号维度有意义
                if key == "credential_name":
                    item["user_email"] = counts["user_email"]
                    item["tier"] = counts["tier"]
                result.append(item)
            result.sort(key=lambda item: -item["total"])
            return result

        # 任务类型聚合：模型名含 image 判为生图任务，其余为普通请求（不改动原始行）
        task_rows = [
            {
                **row,
                "task_type": "image" if "image" in (row.get("model_name") or "").lower() else "chat",
            }
            for row in rows
        ]
        ...
        return {
            "stats": rows,
            "by_credential": aggregate(rows, "credential_name"),
            "by_model": aggregate(rows, "model_name"),
            "by_task_type": aggregate(task_rows, "task_type"),
            "date": date,
        }
    except HTTPException:
        raise
...
async def test_credential(
    ...
    try:
        ...
        response = await post_async(
            ...
            headers=headers,
            timeout=30.0,
            proxy_url=credentials.proxy_url if mode == "antigravity" else ...,
        )
