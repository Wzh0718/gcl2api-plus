"""
OpenAI Responses API Router - GeminiCLI
通过 GeminiCLI 处理 OpenAI Responses API (POST /v1/responses) 请求的路由模块

实现方式：把 Responses 请求转换为 OpenAI chat completions 请求，
复用 openai.py 中现有的 chat_completions 处理管道，再把 chat 响应
翻译回 Responses API 格式。

注意：GeminiCLI 路由当前未在 web.py 中挂载（路由已下线），
本文件为协议 parity 保留，重新挂载后 /v1/responses 即可用。
"""
...
from src.router.geminicli.openai import chat_completions
...
async def responses(
    ...
    log.debug(f"[GEMINICLI-RESPONSES] Request for model: {chat_dict['model']}")
    ...
    result = await chat_completions(openai_request, token)
