    ...
    async def anti_truncation_generator():
        ...
        async def bytes_wrapper():
            ...
            try:
                ...
                # 本生成器被关闭时，级联关闭反截断流
                await aclose_quietly(process_gen, "[GEMINICLI-ANTI-TRUNCATION]")

        # 直接将整个流传递给转换器
        converter_gen = gemini_stream_to_anthropic_stream(
            bytes_wrapper(),
            real_model,
            200
        )
        try:
            async for anthropic_chunk in converter_gen:
                if anthropic_chunk:
                    yield anthropic_chunk
        finally:
            # 客户端断连等导致本生成器被关闭时，级联关闭转换器流
            await aclose_quietly(converter_gen, "[GEMINICLI-ANTI-TRUNCATION]")
