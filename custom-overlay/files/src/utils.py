...
# Antigravity CLI 客户端标识。默认值来自 2026-08-06 实际 AGY CLI 请求，
# 同时允许在 CLI 更新后通过环境变量调整，避免再次把旧版本/旧平台写死。
ANTIGRAVITY_CLI_VERSION = os.getenv("ANTIGRAVITY_CLI_VERSION", "1.1.10")
...
def _extract_flexible_token(
    *,
    authorization: Optional[str],
    x_api_key: Optional[str],
    access_token: Optional[str],
    x_goog_api_key: Optional[str],
    x_anthropic_auth_token: Optional[str],
    anthropic_auth_token: Optional[str],
    key: Optional[str],
) -> Tuple[str, str]:
    candidates = (
        (key, "URL parameter 'key'"),
        (x_goog_api_key, "x-goog-api-key header"),
        (x_anthropic_auth_token, "x-anthropic-auth-token header"),
        (anthropic_auth_token, "anthropic-auth-token header"),
        (x_api_key, "x-api-key header"),
        (access_token, "access_token header"),
    )
    for token, method in candidates:
        if token:
            return token, method
    if authorization:
        ...
        return authorization[7:], "Authorization Bearer header"
    raise HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
...
async def authenticate_flexible(
    ...
    token, auth_method = _extract_flexible_token(
        authorization=authorization,
        x_api_key=x_api_key,
        access_token=access_token,
        x_goog_api_key=x_goog_api_key,
        x_anthropic_auth_token=x_anthropic_auth_token,
...
async def authenticate_antigravity_key(
    request: Request,
    authorization: Optional[str] = Header(None),
    x_api_key: Optional[str] = Header(None, alias="x-api-key"),
    access_token: Optional[str] = Header(None, alias="access_token"),
    ...
    """Authenticate Antigravity calls with a managed key or the legacy env key."""
    # Some Gemini clients incorrectly append the API key as
    # ``...:generateContent/key=<token>`` instead of using ``?key=<token>``.
    # Compatibility routes expose that value as ``path_key``.
    path_key = request.path_params.get("path_key")
    if not key and isinstance(path_key, str) and path_key:
        key = path_key
    ...
    if hmac.compare_digest(token, password):
        ...
        log.debug(f"Antigravity authentication successful using {auth_method} (environment key)")
        return ApiKeyPrincipal("env", "环境配置密钥", "environment", "env")
    try:
        from src.api_keys import ApiKeyAuthError, ApiKeyService
        from src.storage_adapter import get_storage_adapter

        return await ApiKeyService(await get_storage_adapter()).authenticate(token)
    except ApiKeyAuthError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
    except HTTPException:
        raise
    except Exception as exc:
        log.error(f"Managed API key authentication unavailable: {type(exc).__name__}")
        raise HTTPException(status_code=503, detail="密钥认证服务暂不可用") from exc
