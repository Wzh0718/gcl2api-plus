...
async def refresh_access_token_with_clients(
    refresh_token: str,
    proxy_url: Any = ...,
    preferred_client_key: Optional[str] = None,
) -> Tuple[Dict[str, Any], Optional[str]]:
    """
    多 client 降级刷新 access_token（对齐 Antigravity-Manager 的 refresh 语义）

    按候选顺序（preferred → active → 其余）依次尝试注册表中的 OAuth client，
    只有 "client 不匹配" 错误（400/401/403 或 unauthorized_client/invalid_client）
    才尝试下一个；网络错误不降级，直接失败。

    Returns:
        (token_data, 成功使用的 client_key)

    Raises:
        TokenError: 全部 client 均失败
    """
    oauth_base_url = await get_oauth_proxy_url()
    token_url = f"{oauth_base_url.rstrip('/')}/token"

    errors = []
    for client in get_candidates(preferred_client_key):
        data = {
        ...
        try:
            ...
            # 网络错误（连接/超时等）不降级，直接失败
            error_msg = f"刷新请求异常 (client={client.key}): {e}"
            log.error(error_msg)
            raise TokenError(error_msg) from e

        if response.status_code == 200:
            log.debug(f"Token刷新成功 (client={client.key})")
            return response.json(), client.key

        error_text = response.text or ""
        errors.append(f"[{client.key}] HTTP {response.status_code}: {error_text[:200]}")
        if not is_client_mismatch_error(response.status_code, error_text):
            # 非 client 不匹配错误，不再尝试后续 client
            break
        log.warning(f"client {client.key} 不匹配，尝试下一个 client")

    raise TokenError("所有 OAuth client 刷新均失败: " + "; ".join(errors))
...
class Credentials:
    ...
    def _is_antigravity_credential(self) -> bool:
        """判断是否为 antigravity 凭证（走多 client 降级刷新）"""
        if self.oauth_client_key:
            return True
        if self.client_id:
            return any(c.client_id == self.client_id for c in list_clients())
        return False

    def _antigravity_preferred_client_key(self) -> Optional[str]:
        """多 client 刷新的首选 key：优先已记录的 oauth_client_key，其次按 client_id 反查"""
        if self.oauth_client_key:
            return self.oauth_client_key
        for client in list_clients():
            if client.client_id == self.client_id:
                return client.key
        return None

    async def _refresh_antigravity(self):
        """antigravity 凭证的多 client 降级刷新"""
        token_data, client_key = await refresh_access_token_with_clients(
            self.refresh_token,
            proxy_url=self.proxy_url,
            preferred_client_key=self._antigravity_preferred_client_key(),
        )
        ...
        # Google 通常不返回新 refresh_token，未返回时保留原 RT
        if "refresh_token" in token_data:
            self.refresh_token = token_data["refresh_token"]

        # 记录成功使用的 client，后续刷新优先使用
        self.oauth_client_key = client_key
        log.debug(f"Token刷新成功 (client={client_key})，过期时间: {self.expires_at}")
    ...
    async def refresh(self):
        ...
        # antigravity 凭证走多 client 降级刷新
        if self._is_antigravity_credential():
            await self._refresh_antigravity()
            return
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.oauth_client_key:
            result["oauth_client_key"] = self.oauth_client_key
...
async def get_user_info(credentials: Credentials) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = await get_async(
            userinfo_url, headers={"Authorization": f"Bearer {credentials.access_token}"}, proxy_url=credentials.proxy_url
        )
        response.raise_for_status()
...
async def enable_required_apis(credentials: Credentials, project_id: str) -> bool:
    ...
    try:
        ...
        for service in required_services:
            ...
            try:
                check_response = await get_async(check_url, headers=headers, proxy_url=credentials.proxy_url)
            ...
            try:
                enable_response = await post_async(enable_url, headers=headers, json={}, proxy_url=credentials.proxy_url)
...
async def get_user_projects(credentials: Credentials) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        response = await get_async(url, headers=headers, proxy_url=credentials.proxy_url)
...
async def fetch_project_id_and_tier(
    ...
    # 步骤 1: 尝试 loadCodeAssist（tier 在 _try_load_code_assist 内完成映射）
    try:
        project_id, subscription_tier, raw_credit_amount = await _try_load_code_assist(api_base_url, headers, proxy_url=proxy_url)
        ...
        if subscription_tier:
            log.info(
                f"[fetch_project_id_and_tier] loadCodeAssist tier: '{subscription_tier}'"
            )
    ...
    try:
        project_id = await _try_onboard_user(api_base_url, headers, proxy_url=proxy_url)
        if project_id:
            if include_credits:
                return project_id, subscription_tier, credit_amount
            return project_id, subscription_tier
...
def _map_tier(
    ...
    """
    将 loadCodeAssist 返回的 tier name/id 映射为统一 tier（ultra/pro/free）。

    规则（对齐 Antigravity-Manager 的做法，避免 Google 更换 tier id 导致误判）：
    1. name/id 中含 "ultra" -> ultra（最优先，避免 "Google AI Ultra" 被 id 表漏判）
    2. 已知 pro id 或含 "pro" -> pro
    3. standard-tier：付费档按 pro，免费档按 free
    4. 含 "free" -> free
    5. 无法识别：付费档按 pro（确实是付费订阅），免费档返回 None（不谎报）
    """
    candidates = [c.lower() for c in (tier_name, tier_id) if c]
    for t in candidates:
        if "ultra" in t:
            return "ultra"
    for t in candidates:
        if t in ("g1-pro-tier", "helium-tier") or "pro" in t:
            return "pro"
        if t == "standard-tier":
            return "pro" if is_paid else "free"
        if "free" in t:
            return "free"
    return "pro" if is_paid else None
...
async def _try_load_code_assist(
    ...
    # 默认 antigravity 地址时按 sandbox → daily → prod 降级；
    # 自定义地址（如反代）不降级。408/429/5xx 或连接异常才尝试下一个 host，
    # 非 429 的 4xx 直接失败（与原有行为一致）。
    response = None
    last_error: Optional[Exception] = None
    for base_url in antigravity_host_candidates(api_base_url):
        request_url = f"{base_url.rstrip('/')}/v1internal:loadCodeAssist"
        ...
        try:
            ...
            last_error = e
            log.warning(
                f"[loadCodeAssist] {base_url} 连接异常，尝试下一个 host: {type(e).__name__}: {e}"
            )
            continue
        ...
        if response.status_code in (408, 429) or response.status_code >= 500:
            log.warning(
                f"[loadCodeAssist] {base_url} 返回 {response.status_code}，尝试下一个 host"
            )
    ...
    if response.status_code == 200:
        ...
        # 记录账号受限情况（VALIDATION_REQUIRED / RESTRICTED_AGE 等）
        ineligible_tiers = data.get("ineligibleTiers") or []
        if ineligible_tiers:
            reasons = [t.get("reasonCode") for t in ineligible_tiers if isinstance(t, dict)]
            log.info(f"[loadCodeAssist] Account has ineligibleTiers: {reasons}")

        # tier 映射：name 优先（如 "Google AI Ultra"），id 兜底；未知不再谎报为 pro
        subscription_tier = None
        if isinstance(paid_tier, dict) and (paid_tier.get("id") or paid_tier.get("name")):
            subscription_tier = _map_tier(paid_tier.get("name"), paid_tier.get("id"), is_paid=True)
            log.info(
                f"[loadCodeAssist] Found paidTier: name={paid_tier.get('name')!r} "
                f"id={paid_tier.get('id')!r} -> {subscription_tier}"
            )
        elif isinstance(current_tier, dict) and (current_tier.get("id") or current_tier.get("name")):
            subscription_tier = _map_tier(current_tier.get("name"), current_tier.get("id"), is_paid=False)
            log.info(
                f"[loadCodeAssist] Found currentTier: name={current_tier.get('name')!r} "
                f"id={current_tier.get('id')!r} -> {subscription_tier}"
            )
...
async def _try_onboard_user(
    ...
    # 首先需要获取用户的 tier 信息
    tier_id = await _get_onboard_tier(api_base_url, headers, proxy_url=proxy_url)
    if not tier_id:
