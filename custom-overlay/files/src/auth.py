...
def _prepare_credentials_data(credentials: Credentials, project_id: str, mode: str = "geminicli", subscription_tier: str = None, oauth_client_key: str = None) -> Dict[str, Any]:
    """准备凭证数据字典（统一函数）"""
    if mode == "antigravity":
        creds_data = {
        ...
        # RT 登录等多 client 场景下记录成功使用的 OAuth client
        if oauth_client_key:
            creds_data["oauth_client_key"] = oauth_client_key
    else:
        creds_data = {
            "client_id": CLIENT_ID,
...
async def add_antigravity_account_by_refresh_token(
    refresh_token: str, proxy_url: Any = ...
) -> Dict[str, Any]:
    """
    通过 refresh_token 直接导入 Antigravity 账号（RT 登录）

    流程对齐 Antigravity-Manager 的 account_service.add_account：
    1. 多 client 降级刷新换 access_token
    2. userinfo 获取 email
    3. loadCodeAssist/onboardUser 解析 project_id
    4. 保存凭证（含 oauth_client_key）
    """
    try:
        # 步骤1：多 client 刷新换 access_token
        token_data, client_key = await refresh_access_token_with_clients(
            refresh_token, proxy_url=proxy_url
        )

        # Google 刷新时通常不返回新 refresh_token，保留原 RT
        ...
        if not email:
            return {
                "success": False,
                "error": "无法通过 refresh_token 获取用户邮箱，请确认 refresh_token 有效",
            }
        log.info(f"RT 登录获取到邮箱: {email}")

        # 步骤3：解析 project_id 和订阅等级
        antigravity_url = await get_antigravity_api_url()
        project_id, subscription_tier = await fetch_project_id_and_tier(
            credentials.access_token,
            ANTIGRAVITY_USER_AGENT,
        ...
        log.info(f"RT 登录成功: email={email}, project_id={project_id}, client={client_key}")
        return {
            "success": True,
            "credentials": creds_data,
            "file_path": saved_filename,
        ...
        log.error(f"RT 登录失败: {e}")
        return {"success": False, "error": str(e)}


async def save_credentials(creds: Credentials, project_id: str, mode: str = "geminicli", subscription_tier: str = None, oauth_client_key: str = None) -> str:
    """通过统一存储系统保存凭证"""
    ...
    # 准备凭证数据
    creds_data = _prepare_credentials_data(creds, project_id, mode, subscription_tier, oauth_client_key)
