...
async def _close_wreq_response(resp: Any) -> None:
    """尽力关闭 wreq 响应对象，释放底层连接（用于读完错误体后的非流式路径）。"""
    if resp is None:
        return
    try:
        ...
        if callable(close):
            ...
            if inspect.isawaitable(result):
                await result
            return
        aexit = getattr(resp, "__aexit__", None)
        if callable(aexit):
            await aexit(None, None, None)
    except Exception as e:
        log.debug(f"[VERTEX] 关闭 wreq 响应失败: {e}")
    ...
    for attempt in range(max_retries + 1):
        ...
        if resp.status != 200:
            try:
                ...
                err_body = ""
            # 错误体已读完，显式关闭响应释放连接，避免错误重试路径泄漏 fd
            await _close_wreq_response(resp)
