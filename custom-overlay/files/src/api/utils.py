...
from config import (
    ...
    get_max_non_stream_buffer_bytes,
)
from log import log
...
async def record_api_call_success(
    ...
    if credential_manager and credential_name:
        await credential_manager.record_api_call_result(
            ...
            upstream_seconds=upstream_seconds, gateway_seconds=gateway_seconds
        )
...
async def collect_streaming_response(
    ...
    *,
    ...
    Args:
        ...
        minimum_buffer_bytes: 调用方要求的最小缓冲上限。图片模型可用它容纳
            4K Base64 响应，同时仍尊重更大的全局配置。
    ...
    max_buffer_bytes = max(
        await get_max_non_stream_buffer_bytes(),
        max(int(minimum_buffer_bytes), 0),
    )
    ...
    try:
            ...
            try:
                ...
                for part in parts:
                    ...
                    if "functionCall" in part or "functionResponse" in part or "function_call" in part:
                        part_size = len(json.dumps(part, ensure_ascii=False).encode("utf-8"))
                        if buffered_bytes + part_size > max_buffer_bytes:
                            return Response(content=json.dumps({"error": "非流式响应超过内存缓冲上限"}), status_code=413, media_type="application/json")
                    ...
                    if text:
                        ...
                        if buffered_bytes + text_size > max_buffer_bytes:
                            return Response(content=json.dumps({"error": "非流式响应超过内存缓冲上限"}), status_code=413, media_type="application/json")
                        buffered_bytes += text_size
                        # 区分普通文本和思维链
        ...
        # 无论流被完整消费，还是因 [DONE]/错误/缓冲超限/异常提前退出，都显式关闭上游流
        await aclose_quietly(stream_generator, "[STREAM COLLECTOR]")
