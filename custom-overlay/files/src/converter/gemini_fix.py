...
def prepare_image_generation_request(
    ...
    支持多种方式指定图片参数（优先级从高到低）:
    1. Google responseFormat.image（camelCase / snake_case）
    2. Google imageConfig（camelCase / snake_case）
    3. OpenAI 风格顶层 size，如 "1024x1536"
    4. 旧模型名后缀，如 -4k, -2k, -16x9, -1x1
    5. 默认值: 不设置额外参数
    ...
    if generation_config is None:
        generation_config = request_body.pop("generation_config", None)
    generation_config = generation_config.copy() if isinstance(generation_config, dict) else {}

    def normalize_image_config(value: Any) -> Dict[str, str]:
        if not isinstance(value, dict):
            return {}
        result: Dict[str, str] = {}
        aspect_ratio = value.get("aspectRatio", value.get("aspect_ratio"))
        image_size = value.get("imageSize", value.get("image_size"))
        if aspect_ratio:
            result["aspectRatio"] = str(aspect_ratio)
        if image_size:
            result["imageSize"] = str(image_size).upper()
        return result

    response_format = generation_config.pop("responseFormat", None)
    if response_format is None:
        response_format = generation_config.pop("response_format", None)
    response_image = response_format.get("image") if isinstance(response_format, dict) else None

    legacy_image_config = generation_config.pop("imageConfig", None)
    if legacy_image_config is None:
        legacy_image_config = generation_config.pop("image_config", None)

    image_config = normalize_image_config(response_image)
    image_config_source = "responseFormat.image" if image_config else None

    if not image_config:
        image_config = normalize_image_config(legacy_image_config)
        image_config_source = "imageConfig" if image_config else None

    size_str = request_body.pop("size", None)
    if not image_config and isinstance(size_str, str) and size_str.strip():
        image_config = _parse_size_to_image_config(size_str)
        image_config_source = "size" if image_config else None

    if not image_config:
        image_size = None
        for suffix, value in [
            ("-512", "512"), ("-4k", "4K"), ("-2k", "2K"), ("-1k", "1K")
        ]:
        ...
        for suffix, ratio in [
            ...
            ("-8x1", "8:1"), ("-5x4", "5:4"), ("-4x5", "4:5"),
            ("-4x3", "4:3"), ("-4x1", "4:1"), ("-3x4", "3:4"),
            ("-3x2", "3:2"), ("-2x3", "2:3"), ("-1x8", "1:8"),
            ("-1x4", "1:4"), ("-1x1", "1:1"),
        ]:
            if suffix in model_lower:
                aspect_ratio = ratio
        ...
        image_config_source = "model suffix" if image_config else None

    response_modalities = generation_config.pop("responseModalities", None)
    ...
    # Antigravity image backend currently rejects values other than 1.
    generation_config["candidateCount"] = 1
    generation_config["imageConfig"] = image_config

    if image_config_source:
        log.debug(f"[IMAGE] 使用 {image_config_source} 参数: {image_config}")
